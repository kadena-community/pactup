...
use crate::{config::PactupConfig, user_version_reader::UserVersionReader};
...
impl Command for Use {
  ...
  fn apply(self, config: &PactupConfig) -> Result<(), Self::Error> {
    let multishell_path = config
      .multishell_path()
      .ok_or(Error::PactToolboxEnvWasNotSourced)?;
    ...
    let (message, version_path) = if let Some(version) = current_version {
      ...
      {
        let message = format!(
          "Bypassing pactupg {} pact",
          system_version::display_name().cyan()
        );
        (message, system_path)
      } else if alias_path.exists() {
        let message = format!("Using Pact for alias {}", alias_name.cyan());
        (message, alias_path)
      } else {
...
fn will_version_change(resolved_path: &Path, config: &PactupConfig) -> bool {
...
fn warn_if_multishell_path_not_in_path_env_var(
  ...
  outln!(
        ...
        "{} {}\n{}\n",
        "warning:".yellow().bold(),
        "The current Pact path is not on your PATH environment variable.".yellow(),
        "You should setup your shell profile to evaluate `pactup env`, see https://github.com/kadena-community/pactup#shell-setup on how to do this".yellow()
    );
}
...
pub enum Error {
  ...
  #[error(
        "{}\n{}",
        "We can't find the necessary environment variables to replace the Pact version.",
        "You should setup your shell profile to evaluate `pactup env`, see https://github.com/kadena-community/pactup/crates/pactup#shell-setup on how to do this"
    )]
  PactToolboxEnvWasNotSourced,
